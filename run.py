...
def restore_db():
    ...
    with app.app_context():
        ...
        try:
            new_version, original_version, restored_count = restore_backup(filepath)
            if new_version != original_version:
                print(f'数据库恢复成功！数据已从版本 {original_version} 迁移到 {new_version}，共恢复 {restored_count} 条记录')
            else:
                print(f'数据库恢复成功！备份版本: {new_version}，共恢复 {restored_count} 条记录')
        except Exception as e:
            print(f'恢复失败: {e}')
